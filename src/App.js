...
const RightLoginSignupMenu = () => {
  ...
  const {getAccessToken,  state, signIn, signOut } = useAuthContext();

  let isLoggedIn = state.isAuthenticated;
  let token = getAccessToken().then(result => {
  ...
  return (
    <AuthProvider
        config={ {
            ...
            clientID: "XFeXJcDUJB09nvzUAYxdjY_m0GMa",
            baseUrl: "https://api.asgardeo.io/t/wso2apac",
